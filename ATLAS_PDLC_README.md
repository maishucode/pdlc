...
The current pilot bundles three UX Skills for UX specification, React UI delivery, and UX review. The Skill extension model can also support areas such as requirements analysis, Java and Python engineering, security review, testing, architecture, and platform-specific delivery practices. Additional Skills may be provided by the Atlas Team or contributed and governed by an organization's own Discipline owners.

The current Integration Catalog contains the Databricks Integration definition. Organizations can add further governed Integrations and Integration-owned Skills without changing the Core Engine.
...
The current repository contains two executable local Delivery Flows. Availability through a particular coding-agent shortcut can be narrower than the shared Harness capability because Coding Agent Adapters are intentionally thin and may expose only a subset of conversational entry points.

| Delivery Flow | Status | Purpose |
|---|---|---|
| POC | Executable | Validate a bounded idea through approved Requirements, implementation, evidence, and a final disposition. |
| Product Requirements Analysis | Executable, local phase one | Turn an approved product requirement into versioned Story snapshots and an approved Sprint Scope without writing to JIRA or XRAY. |
| Implementation | Planned | Deliver from approved Requirements or Stories through implementation, verification, sign-off, and release integration. |
| End-to-End PDLC | Planned | Coordinate delivery from an initial idea through implementation, release, and outcome review. |
...
`implementation` and `pdlc` are registered as planned definitions but are not executable. They must not be presented or used as available capabilities. Delivery dates are roadmap commitments rather than behavior enforced by the Harness.

The canonical shared Skill accepts the following Product Requirements Analysis entry point when the active Coding Agent Adapter forwards the text to it:

```text
/pdlc product-requirements-analysis <approved product requirement or context>
```

This Flow is local-only in its current phase: it may record optional external identifiers, but it does not create or update JIRA or XRAY work items.
...
Use the official Bun installation method approved by your organization.
...
Organizations that do not permit shell-piped installers should use an approved package manager or an internally distributed Bun binary. Refer to the official Bun installation page for current alternatives.

### Start from the Atlas repository

During the v2 pilot, clone the supported branch explicitly:

```sh
git clone --branch v2 --single-branch https://github.com/maishucode/pdlc.git atlas-poc
cd atlas-poc
...
Confirm that `AGENTS.md`, `.agents/skills/lean-pdlc/SKILL.md`, and the relevant `.codex/` or `.github/` Coding Agent Adapter files exist at their expected repository-relative paths. Restart or reopen the repository after copying the files.
