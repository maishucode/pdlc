...
The Atlas PDLC custom Agent uses the current `.agent.md` filename convention and declares the `read`, `edit`, `search`, `execute`, and native `agent` tool aliases required for a POC and its Stage-scoped capability execution.
...
- Batching every required same-Stage Capability into exactly one generic subagent invocation.
- Reusing a validated execution Receipt while its context hash remains unchanged.
...
- Agent Skills and native generic subagent delegation are enabled where required. Custom Discipline Agents are optional projections, not a runtime dependency.
