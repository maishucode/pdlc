...
This Plugin is an independent, additive extension of the Lean PDLC POC Delivery Flow. It owns one Agent, three Skills, and five Stage bindings. It does not own Delivery Flow state, approvals, or gates.
...
```text
/pdlc poc design and validate a browser feedback classifier
```
...
The manifest opts into the `poc` Delivery Flow and declares the relative location of every contribution type. Agent and Skill identifiers are Plugin-prefixed to prevent collisions.
