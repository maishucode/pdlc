...
- Treat `.pdlc/stages/`, `.pdlc/delivery-flows/`, `.pdlc/domains/`, `.pdlc/roles/`, and `.pdlc/schemas/` as the shared source of truth. Treat `pdlc/config/domains/<domain>/` as the project-specific configuration overlay.
...
- Before every Stage, resolve mandatory Controls, Project Baselines and Defaults, relevant Knowledge, and Capabilities. Auto-apply resolved context instead of asking users to reconfirm it. Keep product requirements confirmation intact, disclose applied context in the final Requirements, and never allow a Project Overlay to weaken enterprise Controls.
