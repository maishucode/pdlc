...
Interpret text appended after `/pdlc` as `<intent> [optional context]`. Currently supported executable intents are `poc`, `resume`, `status`, and `help`. If no intent is supplied, explain these choices in one concise response. Do not offer a planned Delivery Flow as though it were executable.
