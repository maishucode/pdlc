...
2. Inspect the active Delivery Record by reading files. Resolve only the selected Catalog entry, Flow fields, and current Stage definition needed now; do not load the whole Stage Catalog, all Disciplines, or future Stage material into the conversation.
3. If starting and the prompt already contains an idea, do not ask for it again. Create the minimal Requirements shell, prepare only the minimal incomplete DRAFT Record under `pdlc/.state/inbox/<RECORD-ID>.json`, and invoke the controlled initialization operation exactly once. The Runner validates and timestamps the Record, makes it current, appends `DELIVERY_FLOW_CREATED`, and consumes the inbox draft. Never write `pdlc/records/`, `pdlc/.state/current`, or `pdlc/audit/` independently during initialization. If the idea is absent, ask for it immediately and defer file creation. Leave unknown product fields empty rather than inventing success criteria or safety boundaries. Apply Delivery Flow-owned defaults without asking product-requirement questions about role assignment or timebox.
...
- Do not read every role, Discipline asset, Control file, Knowledge document, or Delivery Flow Stage definition at startup. Read returned material only when the current Stage needs it.
...
- Choose `product-requirements-analysis` to approve detailed Requirements, create versioned Story snapshots, and approve a Sprint Scope for downstream Development and QA. Phase one records optional external keys but never writes to JIRA or XRAY.
...
v2 currently executes the POC path and the local Product Requirements Analysis path. Do not simulate unavailable Implementation/PDLC integrations, JIRA/XRAY writes, or production-release behavior.

## Operate Product Requirements Analysis

1. Treat the workspace as one project. Do not add a project-name directory below `pdlc/`; use another workspace for another project. Retain completed Records as shared project history, but allow at most one active Record in a checkout.
2. Start from `.pdlc/examples/requirements-analysis-record.json`. Use the same controlled `init` boundary as POC and never write Record or audit state directly.
3. Obtain explicit Product confirmation at `requirements-approve`, then bind Story artifacts through the Flow-owned `action artifacts-bind` operation (`artifacts bind` remains a compatibility alias) and obtain confirmation at `work-items-ready`.
4. Bind a versioned Sprint Scope containing the Epic reference, Sprint snapshot, and selected Story ids. Obtain explicit Product confirmation at `scope-approve`. This produces a hash-bound handoff and source Git revision; it does not create external work items.
5. Downstream Development or QA must record the upstream Record id, delivered source revision, Scope hash, and the exact selected Story revisions/content hashes. Unselected Stories remain shared context but are not part of that downstream contract.
6. If implementation reveals a defect that does not change approved behavior, keep the Story unchanged. If clarification, Requirements, acceptance criteria, dependency, or Sprint Scope changes, bind a Change Proposal, assess its impact, and obtain Product approval at `change-approve`. Then bind revised Story and Scope artifacts and approve a new Scope version.
7. A changed selected Story blocks downstream progress until explicit rebase. A Scope-only change outside the selected Story set requires acknowledgement but does not invalidate unchanged Story content.

## Operate a POC

1. Read [references/poc-delivery-flow.md](references/poc-delivery-flow.md) before starting or resuming a POC.
2. Find the active Delivery Record under `pdlc/records/`, using `pdlc/.state/current` only as the local selection pointer. Read the Record directly during normal work to avoid unnecessary script approvals.
...
8. Read [references/discipline-context.md](references/discipline-context.md), then use the current Stage's `context` result as the authoritative resolved view. Do not independently scan every Discipline folder. Controls are mandatory. Guidance, References, and KB are advisory context. Defaults are auto-applied unless a higher-precedence source or locked Control prevents an override.
9. Resolve project-specific context only from `pdlc/disciplines/<discipline>/`: approved `baseline.json`, cumulative `policies/`, overrideable `defaults/`, and project `knowledge/`. Project Knowledge must live under `guidance/`, `references/`, or `kb/`, declare its owning Discipline and `appliesTo`, and resolve only when applicable. Never let project content weaken an enterprise Policy/Control.
...
- Route controlled lifecycle operations through the data-driven Flow Engine. Put specialized deterministic behavior in `.pdlc/delivery-flows/<id>/executor.ts` and declare it in the Flow's `runtime`; never add Flow-id dispatch branches to Core or CLI.
- Do not redefine canonical Stage semantics inside a Delivery Flow, Discipline asset, Hook, Integration, or platform adapter. A Stage is not automatically a checkpoint.
...
- On fresh delivery activation, internally invoke the controlled `init` operation exactly once after preparing the Requirements shell and inbox draft. Do not directly create the Record, current pointer, or creation audit event.
...
- Use `context-apply` after a Stage only when its provenance is required by Build Readiness or Verify, or when a material Discipline contribution, Integration, or Policy enforcement was executed. Build Readiness requires receipts for `requirements-clarification` and `build-readiness`; Verify revalidates those receipts and also requires `implementation`, `developer-verification`, `acceptance-verification`, and the active conditional `security-verification` Stage. A local receipt must never delay the first clarification round.
