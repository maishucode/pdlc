...
Build Readiness is the single pre-build human confirmation. It freezes the reviewed Requirements Artifact, binds the complete build contract, and records how each applicable Control is satisfied or formally excepted. The Runner rejects incomplete coverage, too few traceable decisions, unresolved questions or contradictions, draft placeholders, missing review markers, unresolved context conflicts, incomplete business fields, or drift in a previously approved contract.
...
2. `verify`: confirm that Requirements still match their approved content hash and that the whole approved build contract remains intact; validate test, build, demo, and conditionally required security evidence; revalidate current material Stage receipts and Control evidence; then move `COMMITTED → VERIFIED`. Local evidence must resolve to readable regular files inside the workspace. URL and CI references are syntax-checked without a network request.
...
Every controlled Record mutation and its Audit Event are persisted through one Runner operation. If audit persistence fails, the Runner restores the previous Record revision instead of leaving an unaudited transition.
