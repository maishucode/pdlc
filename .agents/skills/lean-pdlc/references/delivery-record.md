...
- Keep `id`, `workflow`, and checkpoint-controlled state stable.
...
- Copy the workflow timebox into the Draft without asking the user. Let the Runner bind all role assignments to the Build Readiness actor when the workflow uses `approval-actor-all-roles`; do not turn these delivery controls into clarification questions or `RQ-xxx` decisions.
- Store evidence as stable file or URL references with a short description; do not embed chat transcripts.
- Record principle applicability and exception references explicitly.
